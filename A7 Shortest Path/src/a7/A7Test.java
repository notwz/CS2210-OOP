...
public class A7Test {
	...
	public void test10OneNodeBoard() {
		...
		List<Node> path= A7.dijkstra(n, n);
	...
	public void test20NoEdges() {
		...
		List<Node> path1= A7.dijkstra(n, n1);
	...
	public void test30TwoNodeOneEdge() {
		...
		List<Node> path2= A7.dijkstra(n1, n);
	...
	public void checkAllShortestPaths(Graph g, PathData pd) {
		for (int r= 0; r < pd.size; r= r + 1) {
			for (int c= 0; c < pd.size; c= c + 1) {
				...
				List<Node> list= A7.dijkstra(pd.nodes[r], pd.nodes[c]);
				assertEquals(pd.dist[r][c], A7.sumOfPath(list));
