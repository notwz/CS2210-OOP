...
/** An instance is a JFrame with a H1N1-spreading tree and a panel to display information
 *
 * @author Mshnik */
public class H1N1JFrame extends JFrame {

    private static final long serialVersionUID= 1L; // Used for serializing

    /** Return a new H1N1Frame to display dt and s, the number of time steps to create it. */
    public static H1N1JFrame show(H1N1Tree dt, int s) {
        return new H1N1JFrame(dt, s);
    }
    ...
    /** H1N1Tree being displayed. */
    private H1N1Tree root;
    ...
    /** GraphPanel that shows the H1N1Tree. */
    ...
    /** Display the H1N1 route from the previously selected person to the selected person. */
    private JLabel twoPersonH1N1RouteLabel= new JLabel();
    ...
    /** The H1N1 route from the previously selected person to the currently selected person. */
    private List<Person> H1N1RoutePrevToSelected;

    /** Constructor: a visible GUI for dt, with s time steps needed to construct it. */
    private H1N1JFrame(H1N1Tree dt, int s) {
        super();
        setTitle("A4 Spreading H1N1");
    ...
    private void createAndAddCircleMouseListener() {
        graphPanel.addCircleMouseListener(new MouseListener() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                try {
                    H1N1RoutePrevToSelected= root.node(previouslySelectedPerson)
                        .H1N1RouteTo(selectedPerson);
    ...
    private void fixGraphPanel(H1N1Tree dt) {
...
//		infoPanel.setLayout(new BoxLayout(infoPanel, BoxLayout.Y_AXIS));
        ...
        infoPanel.add(twoPersonH1N1RouteLabel);
        infoPanel.add(twoPersonCommonAncestorLabel);
    }

    /** Lay out the nodes in the H1N1Tree. Specifically, call recLayoutNodes(...) [below], with
    ...
    private void recLayoutNodes(H1N1Tree dt, float xMin, float xMax, float yPercent,
        float yInc) {
    ...
    /** Return a stroke (for drawing) for line l. If the line is part of the H1N1Route, <br>
     * return a bolded stroke; otherwise, a simple stroke. */
    private Stroke getStrokeFor(GraphPanel<Person, PersonConnection>.Line l) {
        if (H1N1RoutePrevToSelected != null) {
            Person p1= l.getC1().getRepresents();
            Person p2= l.getC2().getRepresents();
            if (H1N1RoutePrevToSelected.contains(p1) &&
                H1N1RoutePrevToSelected.contains(p2)) {
     ...
     * IMMUNE_COLOR */
    private void colorNodes(H1N1Tree dt) {
        GraphPanel<Person, PersonConnection>.Circle c= graphPanel.getCircle(dt.person());
    ...
    private void populateLabels() {
        ...
        if (pp != null) {
            populateLabel(() -> {
                List<Person> route= root.node(pp).H1N1RouteTo(p);
                if (route == null) return null;
                else return Arrays.deepToString(route.stream().map(a -> a.name()).toArray());
            }, twoPersonH1N1RouteLabel, "H1N1 Route");
            populateLabel(() -> {
