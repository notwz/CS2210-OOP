...
/** An instance represents H1N1 spreading and ultimately dying out among<br>
 * a limited population (or killing everyone). <br>
 * <br>
 * Each H1N1 is created on a Network of people and with a chosen first patient. <br>
 * H1N1 is runnable, but for the purposes of this project, it does not need to <br>
 * be run on a separate thread.
 *
 * @author MPatashnik, revised by gries. */
public class H1N1 implements Runnable {
    /** The graph on which this H1N1 is running. */
    private Network network;

    /** The tree representing this H1N1 spreading. */
    private H1N1Tree tree;

    /** Number of time steps this H1N1 took to create dt. */
    private int steps;

    /** The H1N1 model: Statistics that determine the spread of the disease. */
    ...
    /** Constructor: a new H1N1 on network nw with first patient fp and disease model s. */
    public H1N1(Network nw, Person fp, Statistics s) {
        steps= 0;
        network= nw;
        fp.getIll(0);
        tree= new H1N1Tree(fp);
        statistics= s;
    }
    ...
    private void step() {
        ...
        for (Person p : people) {
            if (p.isIll()) {
                ...
                if (n != null && n.isHealthy() && statistics.H1N1SpreadsToPerson()) {
                    n.getIll(steps);
                    tree.insert(p, n);
                }
            }
        ...
        // For each ill person, deduct 1 from health and make dead if health becomes 0
        for (Person p : people) {
            if (p.isIll()) {
    ...
    /** Run H1N1 on the arguments listed in args. <br>
     * If args doesn't match the pattern below, read in arguments via the console by using
     * readArgs().
     *
     * Then, call disease.run() and create an H1N1Frame showing the created H1N1Tree.
     *
    ...
    public static void main(String[] args) {
        ...
        // Set defaults and create the Network, Statistics, and H1N1 objects
        System.out.print("\nSetting up ");
        System.out.print(".");
        ...
        System.out.print(".");
        H1N1 d= new H1N1(n, Util.randomElement(n.vertexSet()), s);
        System.out.println("Done.");
        ...
        H1N1JFrame.show(d.tree, d.steps);
    }
}
